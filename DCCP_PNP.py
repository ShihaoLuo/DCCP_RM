...
def get_pose(Estimater, pose_queue, caculate_pose):
    cap = cv2.VideoCapture(2)
    cap.set(3, 1280)
    cap.set(4, 960)
    while True:
        while caculate_pose.empty():
            time.sleep(0.01)
    ...
    for i in range(nsim):
        ...
        if pose_queue.empty() is False:
            ...
            if PNP_pose[0] is not None:
                ...
                print('before:',pose)
                ang = pose[2]
                pose = pose + np.dot(M.value, (u[:, 0].value))*6
                pose[2] = ang
                # pose[2] = PNP_pose[1]/180*3.1416
                if pose[2] > 3.14159:
...
if __name__ == '__main__':
    ...
    startpoint = np.array([-1, 2])
    endpoint = np.array([1, 2])

    # Parameters for the mpc control
    Ts = 0.25  # sampletime
    DesireSpeed = 0.35 # m/s
    initpose = np.append(startpoint, np.array([0]))
    pose = np.array([0, 0, 0])
    umax = np.array([0.6, 0.6, 0.8])  # ([maxspeed for x, for y , for w])
    umin = np.array([-0.6, -0.6, -0.8])  # ([maxspeed for x, for y , for w])
    dumax = np.array([0.4, 0.4, 0.4])  # the  accelerations of vx,vy,w
    ...
    pose_thread = multiprocessing.Process(target=get_pose, args=(Estimater, pose_queue, caculte_pose,), daemon=True)
    pose_thread.start()
    run(ws, caculte_pose,ref)
    ws.close()
